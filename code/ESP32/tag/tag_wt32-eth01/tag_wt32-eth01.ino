...
void DWM3000Class::init()
{
    ...
    uint32_t ldo_low = readOTP(0x04); //read LDO factory calibrated values from OTP
    ...
    writeSysConfig();
    write(0x00, 0x3C, 0xFFFFFFFF); // enable all events??
    write(0x00, 0x40, 0xFFFF); // make all events generate interrupts???
    write(0x0A, 0x00, 0x000900, 3); //AON config register: on wake go2idle en PGFCAL

    // receiver tuning params for channel 5: manual chapter 8.2.4
    write(0x3, 0x1C, 0x10000240); // channel 5
    ...
    write(0x07, 0x1C, 0x1C071134); // channel 5 recommended PG delay, rest is reserved
    write(0x09, 0x00, 0x1F3C); // PLL config for channel 5
...
void DWM3000Class::writeSysConfig()
{
    ...
    write(GEN_CFG_AES_LOW_REG, 0x10, usr_cfg); // system config: no STS, ...
    ...
    if (config[1] >= 256) // if preamble not 128?
    {
        otp_write |= 0x04;
    }

    write(OTP_IF_REG, 0x08, otp_write); // otp config: channel 5 parameter set
    write(DRX_REG, 0x00, 0x00, 1); // preamble acquisition chunk size (PAC) to 8
    write(DRX_REG, 0x0, config[3]); // same thing kinda
    write(STS_CFG_REG, 0x0, 64 / 8 - 1); // secure timestamp (STS) config
    write(GEN_CFG_AES_LOW_REG, 0x29, 0x00, 1); // undocumented register?
    write(DRX_REG, 0x0C, 0xAF5F584C); // default RX tuning, not optimal value 0xAF5F35CC?
    ...
    write(RF_CONF_REG, 0x1C, rf_tx_ctrl_2); // channel 5 specific value
    write(FS_CTRL_REG, 0x00, pll_conf); // channel 5 specific value
    write(RF_CONF_REG, 0x51, 0x14); // optimal value for LDO load
    write(RF_CONF_REG, 0x1A, 0x0E); //optimal value for TX
    write(FS_CTRL_REG, 0x08, 0x81); // optimal PLL calibration config
    write(GEN_CFG_AES_LOW_REG, 0x44, 0x02); // manually set PPL locked status bit??
    write(PMSC_REG, 0x04, 0x300200); // power management clock config
    write(PMSC_REG, 0x08, 0x0138); // automatic IDLE_RC to IDLE_PLL

    int success = 0;
    for (int i = 0; i < 100; i++)
    {
        if (read(GEN_CFG_AES_LOW_REG, 0x0) & 0x2) // this reads the device ID, not the PLL lock bit???
        {
            success = 1;
            break;
        }
    ...
    write(RX_TUNE_REG, 0x19, 0xF0); // something with RX tuning but weird
    ...
    int l = read(0x04, 0x0C); // RX calibration  value
    delay(20);
    write(EXT_SYNC_REG, 0x0C, 0x11); // reserved value for RX calibration??

    // check RX calibration status
    int succ = 0;
    for (int i = 0; i < 100; i++)
    {
...
{
    return read(0x12, 0x03) & 0b111; // read stage from receive buffer
}
